package org.greencloud.rulescontroller.strategy.defaultstrategy.rules.greenenergy.events.sourcepowershortage;
...
public class ProcessGreenSourcePowerShortageStartEventRule extends
	...
	public void executeRule(final StrategyFacts facts) {
		...
		if (affectedJobs.isEmpty()) {
			...
			affectedJobs.stream().parallel().forEach(serverJob -> {
				final int strategyForJob = agentProps.getStrategyForJob().get(serverJob.getJobInstanceId());
	...
	private void initiatePowerShortageHandler(final List<ServerJob> jobsToTransfer, final StrategyFacts facts) {
		final StrategyFacts handleShortageFacts = new StrategyFacts(facts.get(STRATEGY_IDX));
