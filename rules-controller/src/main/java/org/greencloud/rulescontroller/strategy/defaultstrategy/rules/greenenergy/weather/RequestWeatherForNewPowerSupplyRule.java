package org.greencloud.rulescontroller.strategy.defaultstrategy.rules.greenenergy.weather;
...
public class RequestWeatherForNewPowerSupplyRule extends AgentRequestRule<GreenEnergyAgentProps, GreenEnergyNode> {
	...
	protected ACLMessage createRequestMessage(final StrategyFacts facts) {
		...
		return prepareWeatherCheckRequest(agentProps, job, conversationId, protocol, facts.get(STRATEGY_IDX));
	}
	...
	protected void handleInform(final ACLMessage inform, final StrategyFacts facts) {
		...
		try {
			...
			if (availablePower.isEmpty()) {
				...
				final StrategyFacts offerFacts = new StrategyFacts(facts.get(STRATEGY_IDX));
	...
	private void handleRefusal(final ServerJob job, final StrategyFacts facts) {
