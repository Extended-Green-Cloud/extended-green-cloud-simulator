package org.greencloud.rulescontroller.strategies.defaultstrategy.rules.server.initial;
...
public class StartInitialServerBehaviours extends AgentBehaviourRule<ServerAgentProps, ServerNode> {
	...
	protected Set<Behaviour> initializeBehaviours() {
		return Set.of(
				...
				new ListenForMessages(agent, GREEN_SOURCE_STATUS_CHANGE_RULE, controller),
				...
				new ListenForMessages(agent, JOB_STATUS_CHECK_RULE, controller),
				new ListenForMessages(agent, JOB_MANUAL_FINISH_RULE, controller),
				new ListenForMessages(agent, LISTEN_FOR_POWER_SHORTAGE_FINISH_RULE, controller),
