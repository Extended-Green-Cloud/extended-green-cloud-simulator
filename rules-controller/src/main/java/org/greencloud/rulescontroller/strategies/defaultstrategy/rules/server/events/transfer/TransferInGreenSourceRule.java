package org.greencloud.rulescontroller.strategies.defaultstrategy.rules.server.events.transfer;
...
public class TransferInGreenSourceRule extends AgentCFPRule<ServerAgentProps, ServerNode> {
	...
	private void handleTransferFailure(final StrategyFacts facts) {
		...
		if (nonNull(job)) {
			...
			if (hasStarted) {
				agent.send(prepareJobStatusMessageForCNA(mapToJobInstanceId(job), stateFields.getRight(), agentProps,
