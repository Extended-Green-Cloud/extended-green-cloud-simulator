package org.greencloud.rulescontroller.strategies.defaultstrategy.rules.server.events.errorserver.processing;
...
public class ProcessPowerShortageStartWithAffectedJobsRule extends AgentBasicRule<ServerAgentProps, ServerNode> {
	...
	public void executeRule(final StrategyFacts facts) {
		...
		affectedJobs.forEach(job -> {
			...
			initiatorFacts.put(JOB_ID, mapToJobInstanceId(instances.getSecondInstance()));
		...
		agent.addBehaviour(
				ScheduleOnce.create(agent, facts, HANDLE_POWER_SHORTAGE_RULE, controller, f -> f.get(STRATEGY_IDX)));
	}
}
