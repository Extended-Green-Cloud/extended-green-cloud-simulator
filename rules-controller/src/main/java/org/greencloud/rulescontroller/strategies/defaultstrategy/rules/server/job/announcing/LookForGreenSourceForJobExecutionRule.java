package org.greencloud.rulescontroller.strategies.defaultstrategy.rules.server.job.announcing;
...
public class LookForGreenSourceForJobExecutionRule extends AgentCFPRule<ServerAgentProps, ServerNode> {
	...
	protected void handleRejectProposal(final ACLMessage proposalToReject, final StrategyFacts facts) {
		final ClientJob job = facts.get(JOB);
		agent.send(prepareReply(proposalToReject, mapToJobInstanceId(job), REJECT_PROPOSAL));
	}
	...
	private void handleSelectedProposal(final ACLMessage bestProposal, final StrategyFacts facts) {
		...
		if (agentProps.getServerJobs().containsKey(job)) {
			...
			agent.addBehaviour(
					InitiateProposal.create(agent, FactsMapper.mapToStrategyFacts(facts), PROPOSE_TO_EXECUTE_JOB_RULE, controller));
		} else {
			agent.send(prepareReply(bestProposal, mapToJobInstanceId(job), REJECT_PROPOSAL));
			refuseToExecuteJob(facts);
		}
	}
