package org.greencloud.rulescontroller.strategies.defaultstrategy.rules.cloudnetwork.job.announcing;
...
public class LookForServerForJobExecutionRule extends AgentCFPRule<CloudNetworkAgentProps, CloudNetworkNode> {
	...
	protected void handleRejectProposal(final ACLMessage proposalToReject, final StrategyFacts facts) {
		agent.send(prepareReply(proposalToReject, mapToJobInstanceId(facts.get(JOB)), REJECT_PROPOSAL));
	}
	...
	protected void handleProposals(final ACLMessage bestProposal, final Collection<ACLMessage> allProposals,
		...
		agent.addBehaviour(InitiateProposal.create(agent, FactsMapper.mapToStrategyFacts(facts), PROPOSE_TO_EXECUTE_JOB_RULE, controller));
	}
