package org.greencloud.agentsystem.strategies.basetwostepallocation.rules.regionalmanager.job.allocation.processing;
...
public class ProcessServerSecondAllocationRule extends AgentBasicRule<RegionalManagerAgentProps, RMANode> {
	...
	public void executeRule(final RuleSetFacts facts) {
		...
		final RuleSetFacts dataRequestFacts = constructFactsForJobsAllocationDataRequest(facts.get(RULE_SET_IDX), jobs);
		controller.fire(dataRequestFacts);

		MDC.put(MDC_RULE_SET_ID, valueOf((int) facts.get(RULE_SET_IDX)));
		logger.info("Performing the allocation of the jobs to the servers.");

		final AllocationData allocationData = dataRequestFacts.get(RESULT);
