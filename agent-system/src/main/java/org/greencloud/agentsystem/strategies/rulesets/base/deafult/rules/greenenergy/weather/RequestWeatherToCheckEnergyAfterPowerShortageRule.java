package org.greencloud.agentsystem.strategies.rulesets.base.deafult.rules.greenenergy.weather;
...
public class RequestWeatherToCheckEnergyAfterPowerShortageRule
	...
	protected ACLMessage createRequestMessage(final RuleSetFacts facts) {
		...
		final String conversationId = join("_", job.getJobId(), requireNonNull(job.getStartTime()).toString());
	...
	protected void handleInform(final ACLMessage inform, final RuleSetFacts facts) {
		...
		try {
			...
			if (availableEnergy.isEmpty() || job.getEstimatedEnergy() > availableEnergy.get()) {
				final RuleSetFacts newFacts = constructFactsWithJob(facts.get(RULE_SET_IDX), job);
	...
	private void logJobOnHold(final RuleSetFacts facts) {
		final ServerJob job = facts.get(JOB);
	...
	public AgentRule copy() {
		return new RequestWeatherToCheckEnergyAfterPowerShortageRule(controller);
	}
