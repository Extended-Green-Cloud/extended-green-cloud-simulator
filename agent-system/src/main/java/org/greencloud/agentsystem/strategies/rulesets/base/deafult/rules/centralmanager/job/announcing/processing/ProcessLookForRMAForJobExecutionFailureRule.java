package org.greencloud.agentsystem.strategies.rulesets.base.deafult.rules.centralmanager.job.announcing.processing;
...
public class ProcessLookForRMAForJobExecutionFailureRule extends AgentBasicRule<CentralManagerAgentProps, CMANode> {
	...
	public void executeRule(final RuleSetFacts facts) {
		...
		if (!agentProps.isPossiblyAfterDeadline(job) && retryCounter.get(job.getJobId()).get() < MAX_RETRIES) {
			logger.info("Regional Manager Agents refused to the call for proposal. Putting job back to the queue");
			...
			logger.info("Regional Manager Agents refused to the call for proposal. Sending failure information.");
