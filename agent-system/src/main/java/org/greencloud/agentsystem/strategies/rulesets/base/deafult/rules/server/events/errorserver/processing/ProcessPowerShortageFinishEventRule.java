package org.greencloud.agentsystem.strategies.rulesets.base.deafult.rules.server.events.errorserver.processing;
...
public class ProcessPowerShortageFinishEventRule extends AgentBasicRule<ServerAgentProps, ServerNode> {
	...
	public void executeRule(final RuleSetFacts facts) {
		logger.info("Internal server error has finished! Supplying jobs with power.");
	...
	private void supplyJobWithGreenEnergy(final ClientJob job, final JobInstanceIdentifier jobInstance,
		...
		if (hasStarted) {
			agent.send(prepareJobStatusMessageForRMA(job, GREEN_POWER_JOB_ID, agentProps, facts.get(RULE_SET_IDX)));
		}
	...
	public AgentRule copy() {
		return new ProcessPowerShortageFinishEventRule(controller);
	}
