package org.greencloud.agentsystem.strategies.deafult.rules.greenenergy.events.sourcepowershortage.processing;
...
public class ProcessGreenSourcePowerShortageStartEventRule extends
	...
	private List<ServerJob> getAffectedServerJobs(final Instant startTime) {
		return agentProps.getServerJobs().keySet().stream()
				.filter(job -> nonNull(job.getExpectedEndTime()) && startTime.isBefore(job.getExpectedEndTime()))
	...
	public AgentRule copy() {
		return new ProcessGreenSourcePowerShortageStartEventRule(controller);
	}
