...
public class ProcessServerJobStatusUpdateJobOnHoldRule
	...
	public boolean evaluateRule(final RuleSetFacts facts) {
		return facts.get(MESSAGE_TYPE).equals(ON_HOLD_JOB_ID) && ((boolean) facts.get(JOB_IS_PRESENT));
	}
	...
	public void executeRule(final RuleSetFacts facts) {
		...
		agent.send(prepareJobStatusMessageForCMA(agentProps, jobStatusUpdate, ON_HOLD_JOB_ID, facts.get(RULE_SET_IDX)));
		agentNode.updateGUI(agentProps);
	}

	@Override
	public AgentRule copy() {
		return new ProcessServerJobStatusUpdateJobOnHoldRule(controller);
	}
