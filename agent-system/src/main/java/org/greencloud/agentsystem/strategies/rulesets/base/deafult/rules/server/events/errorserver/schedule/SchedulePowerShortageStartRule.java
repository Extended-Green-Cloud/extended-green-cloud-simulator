package org.greencloud.agentsystem.strategies.rulesets.base.deafult.rules.server.events.errorserver.schedule;
...
public class SchedulePowerShortageStartRule extends AgentScheduledRule<ServerAgentProps, ServerNode> {
	...
	protected void handleActionTrigger(final RuleSetFacts facts) {
		...
		affectedJobs.forEach(job -> {
			if (agentProps.getServerJobs().containsKey(job)) {
				...
				agentProps.getServerJobs().replace(job, ON_HOLD);

				MDC.put(MDC_JOB_ID, jobId);
				MDC.put(MDC_RULE_SET_ID, valueOf((int) facts.get(RULE_SET_IDX)));
				logger.info("Putting job with id {} on hold", jobId);
	...
	public AgentRule copy() {
		return new SchedulePowerShortageStartRule(controller);
	}
