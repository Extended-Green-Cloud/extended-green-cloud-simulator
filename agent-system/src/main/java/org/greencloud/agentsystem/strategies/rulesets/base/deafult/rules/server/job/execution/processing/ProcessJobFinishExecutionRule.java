package org.greencloud.agentsystem.strategies.rulesets.base.deafult.rules.server.job.execution.processing;
...
public class ProcessJobFinishExecutionRule extends AgentBasicRule<ServerAgentProps, ServerNode> {
	...
	private void sendFinishInformation(final RuleSetFacts facts) {
		...
		agentNode.reportJobEnergyUtilization(agentProps, agentProps.getJobsExecutionTime().getForJob(job));
		...
		if (!isJobManuallyFinished && !isExecutedOnBackUpPower) {
			...
			priceFacts.put(MESSAGE, jobFinishMessage);
			agent.addBehaviour(ListenForSingleMessage.create(agent, priceFacts, FINAL_PRICE_RECEIVER_RULE, controller));
		} else {
