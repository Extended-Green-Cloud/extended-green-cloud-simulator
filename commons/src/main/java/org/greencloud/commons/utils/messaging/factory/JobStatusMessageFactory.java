...
public class JobStatusMessageFactory {
	...
	public static ACLMessage prepareJobStatusMessageForClient(final ClientJob job, final String conversationId,
		...
		return prepareJobStatusMessageForClient(job, jobStatusUpdate, conversationId, ruleSet);
	}
	...
	/**
	 * Method prepares the information message about the job execution status sent to the Central Manager Agent
	 *
	 * @param agentProps      properties of Cloud Network Agent
	 * @param jobStatusUpdate details regarding job status update
	 * @param conversationId  type of the message passed to cma
	 * @return INFORM ACLMessage
	 */
	public static ACLMessage prepareJobStatusMessageForCMA(final RegionalManagerAgentProps agentProps,
			final JobWithStatus jobStatusUpdate, final String conversationId, final Integer ruleSet) {
		return prepareJobStatusMessage(jobStatusUpdate, conversationId, ruleSet, agentProps.getCma());
	}

	/**
	 * Method prepares the message about the job changing its status that is sent to the Regional Manager Agent
	 *
	 * @param job            job which update is to be sent
	 * @param agentProps     properties of Server Agent
	 * @param conversationId conversation identifier informing about message type
	 * @return INFORM ACLMessage
	 */
	public static ACLMessage prepareJobStatusMessageForRMA(final ClientJob job, final String conversationId,
			final ServerAgentProps agentProps, final Integer ruleSet) {
		final JobWithStatus jobStatusUpdate = mapToJobWithStatusForCurrentTime(job);
	...
	/**
	 * Method prepares the message about the job being confirmed for execution in the Server
	 * that is sent to the Regional Manager Agent
	 *
	 * @param serverData job execution data
	 * @param job        job for which status is sent
	 * @param ruleSet    job rule set
	 * @param rma        rma to which the message is to be sent
	 * @return INFORM ACLMessage
	 */
	public static ACLMessage prepareJobConfirmationMessageForRMA(final ServerData serverData, final ClientJob job,
			final Integer ruleSet, final AID rma) {
		final JobWithStatus jobWithStatus = ImmutableJobWithStatus.builder()
				.jobId(serverData.getJobId())
				.jobInstanceId(job.getJobInstanceId())
				.priceForJob(serverData.getPriceForJob())
				.newJobExecutionDuration(serverData.getExecutionTime())
				.changeTime(getCurrentTime())
				.build();
		return prepareJobStatusMessage(jobWithStatus, CONFIRMED_JOB_ID, ruleSet, rma);
	}

	/**
	 * Method prepares the message informing the Scheduler that a new client appeared in the system
	 *
	 * @param cma agent identifier of the central manager agent
	 * @param job job that is to be announced
	 * @return INFORM ACLMessage
	 */
	public static ACLMessage prepareJobAnnouncementMessage(final AID cma, final ClientJob job,
			final Integer ruleSet) {
	...
	public static ACLMessage prepareJobStartedMessage(final ClientJob job, final Integer ruleSet,
			final AID... receivers) {
		return prepareJobStatusMessage(job, STARTED_JOB_ID, ruleSet, receivers);
	}
	...
	public static ACLMessage prepareJobFinishMessage(final ClientJob job, final Integer ruleSet,
			final AID... receivers) {
		final JobWithStatus jobWithStatus = mapToJobWithStatusForCurrentTime(job);
