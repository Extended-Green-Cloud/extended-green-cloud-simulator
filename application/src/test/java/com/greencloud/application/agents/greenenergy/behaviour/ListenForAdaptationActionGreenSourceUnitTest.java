	...
	void testAction() {
		...
		verify(greenEnergyAgent).executeAction(
				...
				argThat((data) -> data instanceof IncrementGreenSourceErrorParameters &&
						((IncrementGreenSourceErrorParameters) data).getPercentageChangeUnit() == 5));

		verify(greenEnergyAgent).send(any());

		assertThat(greenEnergyAgent.getWeatherPredictionError()).isEqualTo(0.26);
	}
