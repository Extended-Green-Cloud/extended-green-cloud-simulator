package org.greencloud.strategyinjection.agentsystem.strategy.booking.service;
...
public class LookForRestaurantForClientOrderRule extends AgentCFPRule<BookingProps, BookingNode> {
	...
	protected int compareProposals(final StrategyFacts facts, final ACLMessage bestProposal,
		...
		final StrategyFacts comparatorFacts = new StrategyFacts(facts.get(STRATEGY_IDX));
	...
	protected void handleProposals(final ACLMessage bestProposal, final Collection<ACLMessage> allProposals,
		...
		agentNode.passRestaurantMessageToClient(
				format("""
						...
						""",
						controller.getStrategies().get((int) facts.get(STRATEGY_IDX)).getName(),
